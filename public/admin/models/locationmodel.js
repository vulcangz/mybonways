...
export var LocationModel = {
	...
	NewLocationUpdate: {},
	UpdateNeighbourhood: () => {
		if (!LocationModel.NewLocationUpdate.neighbourhood)
			return new Promise((resolve, reject) =>
				reject("No neighbourhood changes")
			);
	...
	UpdateCountry: () => {
		if (!LocationModel.NewLocationUpdate.country)
			return new Promise((resolve, reject) => reject("No country changes"));
		return m
			.request({
				method: "PUT",
				url:
					"/api/locations/country?country=" +
					LocationModel.LocationUpdate.country,
				data: LocationModel.NewLocationUpdate
			})
			.then(response => {
	...
	UpdateCity: () => {
		if (!LocationModel.NewLocationUpdate.city)
			return new Promise((resolve, reject) => reject("No city changes"));

		return m
			.request({
				method: "PUT",
				url:
					"/api/locations/city?country=" +
					LocationModel.LocationUpdate.country +
					"&city=" +
					LocationModel.LocationUpdate.city,
				data: LocationModel.NewLocationUpdate
			})
			.then(response => {
