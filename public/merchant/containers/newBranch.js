...
var NewBranch = {
	...
	AddBranch: () => {
		if (
			!branch.NewBranch.address ||
			!branch.NewBranch.country ||
			!branch.NewBranch.city ||
			!branch.NewBranch.neighbourhood ||
		...
		branch
			...
			.catch(error => {
				...
				NewBranch.state.NewBranchError =
					"Could not add this branch. Please try again.";
				window.scrollTo(0, 100);
			});
	},
	view: function() {
		...
		branch.NewBranch.latitude = 0.0;
		branch.NewBranch.longitude = 0.0;
		if (place.geometry) {
		...
		return (
			<section class="">
				...
				<section>
						...
						<div class="pa3 pa4-ns bg-white shadow-m2 mt3 cf ">
							...
							<div>
								<input
									type="checkbox"
									class="ph1 dib v-top"
									onclick={m.withAttr("checked", selected => {
										NewBranch.state.useLocation = selected;
									})}
								/>
								<label class="dib v-top">
									save current location as business.<small class="db">
					...
					<div class="w-100 w-50-ns pr2 dib v-top">
						<div class="pa3 pa4-ns bg-white shadow-m2 mt3 cf ">
							<div id="map" class="vh-50 w-100 bg-gray" />
						</div>
					</div>
				</section>
				<div class="pa3 pa4-ns bg-white shadow-m2 mt3 cf">
					<div>
						{NewBranch.state.NewBranchError
							? m(
									"p.white.bg-red-custom.pa1.mv0.tc",
									NewBranch.state.NewBranchError
								)
							: ""}
						{NewBranch.state.NewBranchMessage
							? m(
									"p.white.bg-navy.pa1.mv0.tc",
									NewBranch.state.NewBranchMessage
								)
							: ""}
					</div>
					...
					<div class="pa2">
						<label class="f4 gray pv2 dib">Name:</label>
						<br />
						<input
							type="text"
							class="ba b--light-silver w-100 pa2 bw1"
							value={branch.NewBranch.title}
					...
					<div class="pa2">
						...
						>
							<option disabled selected>
								{Locations.AllNeighbourhoods.length
					...
					<div class="pa2  pv3 mt2 tr">
						<button
							class=" pa2 bg-navy white-90 grow pointer no-underline shadow-4 bw0 w4 "
							onclick={function() {
								// set company id before submission
								branch.NewBranch.company_id = MerchantModel.Merchant.company_id;
								branch.NewBranch.location.company_id =
									MerchantModel.Merchant.company_id;
								NewBranch.AddBranch();
							}}
						>
							{NewBranch.state.loader ? m(".loader") : "Submit Branch"}
						</button>
					</div>
				</div>
			</section>
